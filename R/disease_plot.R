...
plot_disease <- function(pact_disease_table,
  ...
  if (outcome == "frequency") {
    ggplot2::ggplot(
      data = pact_table |>
        ...
        dplyr::mutate(
          grant_type = factor(
            x = grant_type,
            levels = c("n_grants", "n_grants_specified"),
            labels = c(
              "Grants with known\nfinancial commitments",
              "Grants with unspecified\nfinancial commitments"
            )
          )
        ),
      mapping = ggplot2::aes(
      ...
      ggplot2::geom_col(alpha = 0.7, position = ggplot2::position_identity()) +
      ...
      ggplot2::labs(x = "Number of Grants (in thousands)", y = "Disease") +
    ...
    ggplot2::ggplot(
      ...
      ggplot2::labs(
        x = "Financial Commitments (in USD millions)", 
        y = "Disease"
      ) +
      oxthema::theme_oxford() +
      ggplot2::theme(
